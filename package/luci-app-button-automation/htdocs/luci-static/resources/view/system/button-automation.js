...
return view.extend({
	...
	render: function(loadData) {
		...
		m = new form.Map('button_automation', _('按键自动化'), '');
		...
		return m.render().then(function(node) {
			var panel = E('div', {
				...
				E('div', {
					...
					E('label', { 'class': 'cbi-value-title' }, _('监测控制')),
				...
				E('div', {
					...
					E('label', { 'class': 'cbi-value-title' }, _('事件日志')),
					E('div', { 'class': 'cbi-value-field' }, [
						E('textarea', {
							'id': 'btn-monitor-log',
							'readonly': 'readonly',
			...
			node.addEventListener('change', function(ev) {
				...
				if (!target || target.closest('#btn-monitor-log') || target.closest('#btn-monitor-toggle') || target.closest('#btn-monitor-clear'))
					return;
				scheduleSave(180);
			}, true);
